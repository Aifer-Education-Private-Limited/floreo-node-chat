...
// ======================
// /clientOrTherapy NAMESPACE
// ======================
const clientOrTherapyIO = io.of("/clientOrTherapy");

clientOrTherapyIO.on("connection", (socket) => {
  console.log("clientOrTherapy Connected:", socket.id);
  ...
  socket.on("joinChat", async (d) => {
    ...
    const res = await fetch(`${BASE_URL}/api/method/floreo.api.client_app.chat.join_chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ users, subject }),
    });
    ...
    const msgRes = await fetch(`${BASE_URL}/api/method/floreo.api.client_app.chat.get_messages`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chat_id: chatId }),
    });
  ...
  const res = await fetch(`${BASE_URL}/api/method/floreo.api.client_app.chat.send_message`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body :JSON.stringify({ message, sourceId, chatId, isImage, isVoice, isPDF }),
  });
  ...
  clientOrTherapyIO.to(msg.chatId).emit("OneByOnemessage", data.message);
});
